...
pub fn match_symbols(old_symbols: &[Symbol], new_symbols: &[Symbol]) -> MatchResult {
    ...
    for (oi, old_sym) in old_symbols.iter().enumerate() {
        ...
        for (ni, new_sym) in new_symbols.iter().enumerate() {
            ...
            // Use AST structural similarity when fingerprints are available (O(n+m)),
            // falls back to text-based Levenshtein when not available
            let body_sim = old_sym.structural_similarity_threshold(new_sym, body_threshold);
